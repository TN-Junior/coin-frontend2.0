...
function Table() {
  ...
  const carregarContas = async () => {
    try {
      const response = await axios.get('coin-backend-production-5d52.up.railway.app/api/contas');
  ...
  const adicionarOuEditarConta = async () => {
    ...
    try {
      ...
      if (isEditing) {
        const response = await axios.put(`coin-backend-production-5d52.up.railway.app/api/contas/${editId}`, contaParaEnviar);
        setContas(contas.map(conta => (conta.id === editId ? response.data : conta)));
      } else {
        const response = await axios.post('coin-backend-production-5d52.up.railway.app/api/contas', contaParaEnviar);
  ...
  const deletarConta = async (id) => {
    try {
      await axios.delete(`coin-backend-production-5d52.up.railway.app/api/contas/${id}`);
