...
const Pagamentos = () => {
  ...
  const loadPagamentos = async (isReceitas) => {
    try {
      ...
      const response = await axios.get(`coin-backend-production-5d52.up.railway.app/api/pagamentos/${tipo}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (pagamentoAtual) {
        await axios.put(`coin-backend-production-5d52.up.railway.app/api/pagamentos/${pagamentoAtual.id}`, pagamento);
      } else {
        await axios.post('coin-backend-production-5d52.up.railway.app/api/pagamentos', pagamento);
      }
      loadPagamentos(isReceitas);
      closeModal();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`coin-backend-production-5d52.up.railway.app/api/pagamentos/${id}`);
      loadPagamentos(isReceitas);
    } catch (error) {
